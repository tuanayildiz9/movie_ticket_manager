...
class KundeRepository:
    def _to_domain(self, session: Session, row: KundeORM) -> Kunde:
        ...
        return Kunde(
            ...
            account_id=row.account_id or row.kunde_id,
            vorname=row.vorname,
    ...
    def get_by_account_id(self, account_id: UUID) -> Kunde | None:
        with Session(engine) as session:
            row = session.exec(select(KundeORM).where(KundeORM.account_id == account_id)).first()
            return self._to_domain(session, row) if row else None
    ...
    def update(self, kunde: Kunde) -> Kunde:
        with Session(engine) as session:
            ...
            row.account_id = kunde.account_id
