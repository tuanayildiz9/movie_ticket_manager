...
def generate_ticket_pdf(bestellung, snacks_map: dict, film_vorstellung_map: dict, sitzplatz_map: dict) -> bytes:
    """
    ...
    film_vorstellung_map : {vorstellung_id: {"titel": str, "startzeit": datetime, "ort": str, "saal": str}}
    sitzplatz_map  : {sitzplatz_id: {"sektor": str, "sitz_label": str}}
    """
    buffer = BytesIO()
    ...
    # ── Header ──────────────────────────────────────────────────────────────
    story.append(Paragraph("🎬 MoviTicket", s["title"]))
    ...
    datum = bestellung.bestellungsdatum.strftime("%d.%m.%Y %H:%M")
    meta_data = [
    ...
    for i, ticket in enumerate(bestellung.tickets, 1):
        ...
        ticket_rows = [
            ...
            ["Sitzplatz:", f"Reihe {sitz.get('sektor', '–')}, Platz {sitz.get('sitz_label', '–')}"],
            ["Vergünstigung:", disc_label],
            ["Ticketpreis:", f"CHF {ticket.preis:.2f}"],
        ]

        # Snacks für dieses Ticket
        if ticket.snacks:
            snack_lines = []
            for ts in ticket.snacks:
                snack = snacks_map.get(ts.snack_id)
                if snack:
                    snack_lines.append(f"{snack.name} ×{ts.anzahl}")
            if snack_lines:
                ticket_rows.append(["Snacks:", ", ".join(snack_lines)])
    ...
    story.append(Paragraph(f"MoviTicket · Bestellung {bid_short}", s["small"]))
